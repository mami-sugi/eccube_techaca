...
class LC_Page_Admin_Contents extends LC_Page_Admin_Ex
{
    ...
    public function action()
    {
        ...
        switch ($this->getMode()) {
            ...
            case 'pre_edit':
                ...
                list($news['finish_year'],$news['finish_month'],$news['finish_day']) = $this->splitNewsDate($news['cast_finish_date']);
    ...
    public function lfInitParam(&$objFormParam)
    {
        ...
        $objFormParam->addParam('表示終了期限(年)', 'finish_year', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('表示終了期限(月)', 'finish_month', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
        $objFormParam->addParam('表示終了期限(日)', 'finish_day', INT_LEN, 'n', array('EXIST_CHECK', 'NUM_CHECK', 'MAX_LENGTH_CHECK'));
    ...
    public function doRegist($news_id, $sqlval, SC_Helper_News_Ex $objNews)
    {
        ...
        $sqlval['finish_date'] = $this->getFinishDate($sqlval);
        unset($sqlval['year'], $sqlval['month'], $sqlval['day'],$sqlval['finish_year'], $sqlval['finish_month'], $sqlval['finish_day']);
     ...
     * @return string 登録日を示す文字列
     */
    public function getFinishDate($arrPost)
    {
        $finishDate = $arrPost['finish_year'] .'/'. $arrPost['finish_month'] .'/'. $arrPost['finish_day'];
